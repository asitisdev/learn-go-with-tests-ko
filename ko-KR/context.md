...
TDD에서 규율을 유지하는 것을 기억하세요. 테스트를 통과시키기 위해 _최소한_의 코드를 작성하세요.
...
이것은 이 테스트를 통과시키지만 기분이 좋지 않죠! _모든 요청_에서 fetch 전에 `Cancel()`을 취소해서는 안 됩니다.
...
일부 엔지니어들은 _편리하게 느껴지므로_ `context`를 통해 값을 전달하는 것을 옹호해 왔습니다.
