...
다른 숫자로 또 다른 테스트를 작성하여 해당 테스트가 실패하도록 강제할 수 있지만, [고양이와 쥐 게임](https://en.m.wikipedia.org/wiki/Cat*and*mouse)처럼 느껴집니다.
...
예제 함수는 `Example`로 시작하며(테스트 함수가 `Test`로 시작하는 것처럼), 패키지의 `*test.go` 파일에 있습니다. 다음 `ExampleAdd` 함수를 `adder*test.go` 파일에 추가하세요.
